...
## 3.5 La misma pregunta sin proyectar: el ERGM bipartito híbrido, por comisión y por bloque
...
Sobre esa red estimamos un ERGM (exponential random graph model), que conviene explicar en una frase: es un modelo de probabilidad sobre la red completa, donde cada término es un *contador* de un patrón, y su coeficiente pregunta "¿este patrón aparece en la red real más (o menos) que en redes aleatorias comparables?" — positivo, el patrón sobra respecto del azar; negativo, falta. La versión que reportamos aquí es *híbrida*: combina la estructura completa del ERGM con la pregunta de heterogeneidad que naturalmente se le hace al logit condicional ("¿y esto es igual en todos los sectores políticos?"). La clave de la combinación es que, dentro de un ERGM, "separar por bloque" nunca es cortar la red — cortar mutila coaliciones y rompe el objeto modelado —, sino **multiplicar los contadores**: la red se modela entera, y las estadísticas se especializan por bloque. Los bloques son los mismos cinco del logit por bloques: Derecha (Vamos por Chile, 37), Centro-izq (Lista del Apruebo + INN, 28), Izquierda (Apruebo Dignidad + Lista del Pueblo, 51), PPOO (escaños reservados, 17) y Otras (listas locales, 21). Los términos, en cuatro familias:
...
- **Los contadores de perfil por bloque** — la parte clogit del híbrido. Para cada atributo de perfil (abogado, experiencia, género, distrito), el contador clásico de homofilia ("pares de co-firmantes que comparten el estatus") se **particiona exactamente en seis**: los pares donde ambos miembros son del mismo bloque (cinco términos, uno por bloque) y los pares que cruzan bloques (un término). La suma de los seis reproduce el término global de la especificación anterior; la partición deja ver dónde vive cada homofilia.
...
En forma completa: sea $y$ la red bipartita de la comisión ($y_{ia} = 1$ si el convencional $i$ firmó la iniciativa $a$; $i = 1,\dots,154$, $a = 1,\dots,n_2$). El modelo es

$$\Pr(Y = y) \;=\; \frac{\exp\big(\theta^\top s(y)\big)}{\kappa(\theta)}, \qquad \kappa(\theta) = \sum_{y' \in \mathcal{Y}} \exp\big(\theta^\top s(y')\big),$$

donde $\kappa(\theta)$ suma sobre todas las redes bipartitas posibles y $s(y)$ es el vector de estadísticas — los contadores. Con $b_i \in \{$Derecha, Centro-izq, Izquierda, PPOO, Otras$\}$ el bloque de $i$, $m_i$ el indicador de membresía en la comisión, $x_i$ un atributo de perfil y $v_i$ una variable continua, los componentes de $s(y)$ son:

$$s_{\text{edges}} = \sum_{i,a} y_{ia}; \qquad s_{\text{miembro}} = \sum_{i,a} y_{ia}\, m_i; \qquad s_{\text{rango}}(v) = \sum_{a} \Big(\max_{i:\,y_{ia}=1} v_i - \min_{i:\,y_{ia}=1} v_i\Big)$$

(cuatro rangos globales: $\theta_1$, $\theta_2$, edad, grado);

$$s_{\text{mezcla}}^{(g,h)} = \sum_{a} \sum_{i<j} y_{ia}\, y_{ja}\, \mathbf 1\{b_i, b_j\} {=} \{g, h\} \qquad \text{(15 celdas, pares no ordenados de bloques)};$$

$$s_{\text{perfil}}^{(X,\, g)} = \sum_{a} \sum_{i<j} y_{ia}\, y_{ja}\, \mathbf 1[x_i = x_j]\, \mathbf 1[b_i = b_j = g], \qquad s_{\text{perfil}}^{(X,\, \text{cross})} = \sum_{a} \sum_{i<j} y_{ia}\, y_{ja}\, \mathbf 1[x_i = x_j]\, \mathbf 1[b_i \neq b_j]$$

(para cada $X \in \{$abogado, experiencia, género, distrito$\}$: cinco contadores dentro-de-bloque más el complemento entre-bloques; su suma reproduce exactamente el contador global de homofilia de la especificación anterior);

$$s_{\text{disciplina}}^{(g)} = \sum_{a} \Big(\max_{i:\,y_{ia}=1,\, b_i=g} \theta_{1i} - \min_{i:\,y_{ia}=1,\, b_i=g} \theta_{1i}\Big) \qquad \text{(el rango del contingente del bloque } g\text{, cero si tiene} < 2 \text{ firmantes)};$$

y los dos términos geométricamente ponderados con decaimiento fijo $\alpha = 0.5$:

$$s_{\text{gwb1deg}} = e^{\alpha} \sum_{k=1}^{n_2} \Big(1 - (1 - e^{-\alpha})^{k}\Big)\, D_k(y), \qquad s_{\text{gwdsp}} = e^{\alpha} \sum_{k \geq 1} \Big(1 - (1 - e^{-\alpha})^{k}\Big)\, P_k(y),$$

donde $D_k(y)$ es el número de convencionales con exactamente $k$ firmas y $P_k(y)$ el número de díadas (de cualquier tipo) con exactamente $k$ socios compartidos — pares de personas con $k$ documentos en común y pares de documentos con $k$ firmantes en común. La ponderación $1 - (1-e^{-\alpha})^k$ crece con retornos decrecientes: la firma (o el socio compartido) número 30 aporta casi nada más que el número 10, que es lo que evita la degeneración de los contadores crudos.
...
Cómo leer el conjunto. Primero, los controles se comportan como en todas las especificaciones anteriores: la pertenencia multiplica la firma en las siete comisiones ($+0.79$ a $+2.20$), la ideología global opera continua en ambas dimensiones (rangos negativos por documento), y las *parejas repetidas* siguen faltando en las siete ($-0.16$ a $-0.47$) — sobre esa firma estructural volvemos abajo. Segundo, la matriz de mezcla: con los contadores de perfil dentro del modelo, el cierre de bloque "a secas" se modera respecto de la lectura sin partición (la especificación de mezcla pura, sin contadores de perfil, está en `M1_bipartite_nodemix_boot.csv`: allí la diagonal PPOO/Otras/Centro-izq es positiva en 5 de 7 y la derecha nula — la cohesión de la derecha *es* su compacidad ideológica); lo que la matriz conserva nítido aquí es la sub-mezcla con PPOO (celdas cruzadas negativas o nulas casi siempre) y la rareza conocida de C4, donde toda la estructura de bloques se apaga.

Tercero — el corazón del híbrido — la partición del perfil por bloque:

1. **El territorio es de la derecha por dentro, y un puente por fuera.** Compartir distrito organiza la co-firma *dentro* de la Derecha ($+0.41$ a $+1.36$, significativo en 4 de 7, positivo siempre) y la *des*-organiza dentro de la Izquierda ($-0.45$ a $-0.64$, negativo y significativo en 4): la izquierda organizada reparte sus firmas entre distritos — coordinación nacional, no territorial. Y la fila *entre bloques* es positiva y significativa en las siete ($+0.28$ a $+0.67$): el canal territorial vive sobre todo *cruzando* los bloques políticos. Es la conclusión del logit por bloques (sección 3.1), reproducida ahora neta de cierre de bloque, ideología continua en 2D y estructura. Y la composición territorial descarta la lectura mecánica ("la derecha estaba concentrada y no tenía donde elegir"): es al revés. La derecha es, en proporción, el bloque más repartido — 37 personas en 26 distritos, con una sola delegación de cuatro (el D11, el oriente de Santiago) y dos duplas en la Araucanía — y apenas 14 pares posibles de mismo distrito (0.38 por persona); la izquierda dobla candidatos en 15 distritos y dispone de 42 pares posibles (0.82 por persona, el triple). El contraste es de *uso*, no de oportunidad: la derecha explota intensamente sus pocas duplas territoriales — con el oriente de Santiago a la cabeza — y la izquierda, con el triple de cancha, las suprime.
2. **El nulo de los abogados, con resolución de bloque.** En *ningún* bloque compartir el título organiza la firma: señales sueltas, chicas y sin patrón (la más "fuerte": $-0.15$ dentro de la Derecha en C2). La pregunta "¿cuánto importa ser abogado según el sector?" tiene respuesta uniforme: en ninguno.
3. **La experiencia vive en la izquierda.** Pares experimentados de izquierda se agrupan ($+0.11$ a $+0.13$, significativo en C2 y C4, positivo en cinco comisiones) — en ningún otro bloque pasa. Chico pero consistente con el eco de experiencia del logit condicional.
4. **El género opera entre bloques, no dentro.** Dentro de los bloques el mismo-género es nulo o negativo — fuertemente negativo en PPOO ($-0.60$ a $-1.64$), casi seguro un reflejo institucional de sus candidaturas paritarias emparejadas — mientras que *entre* bloques es positivo, chico y consistente ($+0.08$ a $+0.20$, significativo en cinco): cuando la firma cruza el clivaje político, el género la ayuda a cruzar.
5. **Disciplina ideológica interna.** Donde es significativo, el rango del contingente propio es negativo — Izquierda en C4/C5 ($-3.7$/$-3.8$), Otras en tres comisiones, Derecha en C4 —: los bloques suman a sus documentos gente que *no estira* su rango. Las delegaciones se arman compactas, y la global ($\theta_1$ por documento) ya mostraba que el documento entero también.

Dos cautelas de lectura, dichas antes de que el lector las descubra. Los coeficientes explosivos (Experiencia — dentro de Otras en C3 y C5: $+11$, $+8$) son cuasi-separación en celdas minúsculas — el bloque Otras casi no tiene pares experimentados ahí — y deben leerse como "sin información", igual que las celdas "---". Y la partición reparte lo que antes era un solo número: los términos de mezcla y los contadores de perfil se reparten la señal que la especificación anterior concentraba en "misma lista" y las homofilias globales, así que las magnitudes no son comparables uno-a-uno con esa versión; la comparación válida es entre bloques dentro de esta misma tabla.

Y el bloque estructural, que no cambió con el híbrido — señal de robustez. Los *firmadores seriales* (`gwb1degree`) siguen heterogéneos (concentración residual fuerte en C4 y C5, lo contrario en C6): no hay un patrón universal de desigualdad más allá de los atributos. Las *parejas repetidas* (`gwdsp`) dan, otra vez, el resultado más nítido: negativo y significativo en las siete comisiones. El coeficiente negativo dice esto: tomada la red real de una comisión y comparada contra redes con la *misma* actividad por persona, la *misma* mezcla de bloques y las *mismas* homofilias de perfil, la red real tiene **menos parejas insistentes y menos documentos con paquetes de firmantes casi idénticos** de lo que esa referencia produciría. Traducido al proceso político: cuando se armaba una nueva iniciativa, no se re-usaba la coalición de la anterior — se re-armaba con pares frescos. Y la interpretación institucional es quizás la más elegante del bloque: es la firma estructural de la tabula rasa. En una legislatura con décadas de historia, las duplas de trabajo están atrincheradas — socios de comité de toda la vida, parejas de proyecto que se repiten congreso tras congreso — y un `gwdsp` positivo sería lo esperable. Aquí nadie se conocía con nadie: no existía el *stock* de sociedades heredadas, y el modelo lo detecta como ausencia de inercia relacional (aun cuando la familiaridad ya empezaba a operar en el margen, como muestra el RHEM ordenando la secuencia). ¿Contradice esto al RHEM (3.3), donde la familiaridad de pares es el mejor predictor? No — y el contraste es instructivo. El RHEM mira la *película*: compara cada coalición real contra coaliciones ficticias del mismo día, y encuentra que la familiaridad ordena la *secuencia* (quién arma el próximo equipo). El `gwdsp` mira la *foto final*: compara el stock total de repetición contra la referencia del propio modelo, y encuentra que ese stock no es excesivo. Ambas cosas son ciertas a la vez gracias a la regla 8--16: cada iniciativa obliga a coaliciones anchas, así que hasta las duplas más fieles llegan siempre acompañadas de pares nuevos — los mismos capitanes, con el equipo renovándose en cada partido.

La relación con 3.1 es de triangulación, no de redundancia — y con el híbrido, la triangulación es de a dos preguntas. El logit condicional pregunta por la *elección* frente a un menú concreto y, en su versión por bloques, por cómo decide cada sector; el ERGM híbrido pregunta por la *estructura agregada* de cada pool de documentos y, con sus contadores por bloque, por cómo se combinan los sectores dentro de las coaliciones reales. Que lista/bloque, ideología y territorio-condicionado organicen la firma en las dos vistas — y en la dinámica del RHEM — permite afirmar la homofilia como propiedad del proceso y no como artefacto de un modelo; y que la heterogeneidad por bloque (el territorio de la derecha, la experiencia de la izquierda, el género entre bloques) sobreviva el paso del menú a la red completa la afirma como propiedad de los *sectores*, no de la herramienta.
