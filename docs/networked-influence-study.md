...
Y el bloque nuevo, la estructura. Los *firmadores seriales* (`gwb1degree`) son heterogéneos: concentración residual de actividad fuerte en C4 y C5, lo contrario en C6 — no hay un patrón universal de desigualdad más allá de los atributos. Las *parejas repetidas* (`gwdsp`), en cambio, dan el resultado más nítido del bloque: negativo y significativo en las siete comisiones — condicional en quién firma cuánto y en toda la homofilia, las duplas no se sobre-usan ni los documentos clonan paquetes de firmantes: las coaliciones se re-arman una y otra vez con pares frescos. ¿Contradice esto al RHEM (3.3), donde la familiaridad de pares es el mejor predictor? No — y el contraste es instructivo. El RHEM mira la *película*: compara cada coalición real contra coaliciones ficticias del mismo día, y encuentra que la familiaridad ordena la *secuencia* (quién arma el próximo equipo). El `gwdsp` mira la *foto final*: compara el stock total de repetición contra la referencia del propio modelo, y encuentra que ese stock no es excesivo. Ambas cosas son ciertas a la vez gracias a la regla 8--16: cada iniciativa obliga a coaliciones anchas, así que hasta las duplas más fieles llegan siempre acompañadas de pares nuevos — los mismos capitanes, con el equipo renovándose en cada partido.
