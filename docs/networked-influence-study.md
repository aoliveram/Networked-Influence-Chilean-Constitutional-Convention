...
- *misma lista, mismo distrito, ambos abogados, ambos con experiencia, mismo género* (`b1nodematch`): pares de co-firmantes de una misma iniciativa que comparten el atributo — homofilia de co-firma sin proyección, para las variables genuinamente categóricas o binarias;
- *rango de $\theta_1$, de $\theta_2$, de edad y de grado por documento* (`b2covrange`): cuán dispersos son los firmantes de cada texto en cada variable continua — negativo = los textos concentran firmantes parecidos;
- y dos términos *estructurales*, que capturan dependencia pura de red más allá de todo atributo. El primero, `gwb1degree`, pregunta si hay *firmadores seriales*: ¿la distribución de firmas por persona está más concentrada (o más pareja) de lo que los atributos predicen? La ponderación geométrica (el "gw") es el truco anti-explosión: la firma n.º 30 de la misma persona cuenta menos que la n.º 2, así el término mide la forma de la distribución sin que los mega-firmantes lo dominen. El segundo, `gwdsp` (socios compartidos por díada, también con descuento geométrico), pregunta si las *parejas se repiten*: para cada par de convencionales cuenta los documentos que ambos firmaron (y para cada par de documentos, los firmantes que comparten), premiando menos cada repetición adicional. Positivo = la red fabrica duplas insistentes y textos gemelos en exceso; negativo = condicional en quién firma cuánto y con qué atributos, las firmas se reparten. (Una diferencia de forma con el logit condicional y el RHEM: allá las variables continuas entran como distancias — $|\theta_i - \bar\theta|$, dispersión de edad por pares —; en el ERGM bipartito la homofilia estándar entre co-firmantes es por *coincidencia de categoría* (`b1nodematch`), así que ideología, educación y edad entran discretizadas. No es una limitación del ERGM en general — en redes de un modo existe `absdiff`, la diferencia continua sobre el lazo — sino del vocabulario de dos-caminos bipartito; la alternativa continua nativa que sí existe es el *rango* por documento (`b2covrange`: cuán dispersos son los firmantes de cada texto en una variable), y la Tabla 8 la incluye para ideología, educación y edad. La paridad entre los tres modelos es de variables, no de forma funcional.)

¿Por qué siete modelos y no uno? Por dos razones, una sustantiva y una práctica. La sustantiva: la comisión es el mayor confundidor de composición (la sección 3.4 mostró que los temas arman las coaliciones), y estimar dentro de cada comisión es condicionar por ese confundidor por diseño — en un modelo único de las 947 iniciativas, con un solo intercepto, las diferencias de composición *entre* comisiones contaminan la homofilia *dentro* de cada una (lo verificamos: el modelo agregado invierte los signos de la homofilia, la paradoja de Simpson de manual). La práctica: la estimación MCMC completa resultó computacionalmente infactible en estas redes (documentado); la tabla usa máxima pseudo-verosimilitud (MPLE) — que entrega los puntos en segundos — con errores estándar por *bootstrap de iniciativas* (B = 500: re-sortear las iniciativas de la comisión con reemplazo, reconstruir la red y re-estimar), que corrigen el optimismo de los EE logísticos ingenuos (inflación medida de 1.2 a 3.6 veces). Los siete modelos ($^{*}$ $p<.05$, $^{**}$ $p<.01$, $^{***}$ $p<.001$; errores estándar completos en `M1_bipartite_commissions.csv`):

**Tabla 8 — ERGM bipartito por comisión (siete modelos; variables continuas solo en forma continua, más dos términos estructurales; estimación MPLE con EE por bootstrap de iniciativas, B = 500).**
...
| Rango de grado por documento | $-0.26$ | $-0.59^{**}$ | $-0.27$ | $+0.05$ | $-0.39^{*}$ | $-0.33$ | $-0.01$ |
| Rango de edad por documento (décadas) | $-0.28$ | $+0.15$ | $-0.22$ | $+0.02$ | $-0.49^{**}$ | $+0.02$ | $+0.16$ |
| ***— Estructura (más allá de los atributos)*** | | | | | | | |
| Firmadores seriales (`gwb1degree`) | $-0.57$ | $-0.75$ | $+0.04$ | $-8.04^{*}$ | $-1.92^{***}$ | $+1.34^{**}$ | $-0.49$ |
| Parejas repetidas (`gwdsp`) | $-0.42^{***}$ | $-0.28^{***}$ | $-0.38^{***}$ | $-0.16^{***}$ | $-0.23^{***}$ | $-0.35^{***}$ | $-0.24^{***}$ |

Cómo leer el conjunto, ahora con la inferencia honesta (todas las estrellas son bootstrap). Primero, la pertenencia: en las siete comisiones, ser miembro multiplica la propensión a firmar los textos de esa comisión ($+0.79$ a $+2.24$). Segundo — el resultado central — la ideología opera continua y en *ambas* dimensiones: los textos evitan la dispersión tanto en el eje clásico ($-2.0$ a $-3.6$, las 7 comisiones) como en el segundo eje, el que separa a los escaños reservados ($-0.7$ a $-2.4$, significativo en 6) — con la cautela de que el rango crece mecánicamente con el tamaño del documento. Tercero, la lista sobrevive pero se achica al condicionar en estructura ($+0.06$ a $+0.23$): parte de lo que parecía coordinación de lista era, en realidad, la estructura de actividad de sus miembros. Cuarto, el territorio es robusto en seis comisiones ($+0.16$ a $+0.43$; nulo en C4, el pool más masivo y transversal). Quinto, el perfil personal casi no organiza nada: los abogados no se buscan (con la excepción curiosa de C6-Justicia, $+0.08$ — la única comisión donde son mayoría absoluta), la experiencia suma poco, y género y edad quedan *neutros* bajo la especificación completa — la lectura de "mezcla" que sugerían las especificaciones sin estructura no sobrevive; solo la educación conserva una mezcla débil (rango negativo en 2-3 comisiones).

Y el bloque nuevo, la estructura. Los *firmadores seriales* (`gwb1degree`) son heterogéneos: concentración residual de actividad fuerte en C4 y C5, lo contrario en C6 — no hay un patrón universal de desigualdad más allá de los atributos. Las *parejas repetidas* (`gwdsp`), en cambio, dan el resultado más nítido del bloque: negativo y significativo en las siete comisiones. En concreto, el coeficiente negativo dice esto: tomada la red real de una comisión y comparada contra redes que tienen la *misma* actividad por persona y la *misma* homofilia de lista, ideología y distrito, la red real tiene **menos parejas insistentes y menos documentos con paquetes de firmantes casi idénticos** de lo que esa referencia produciría. Traducido al proceso político: cuando se armaba una nueva iniciativa, no se re-usaba la coalición de la anterior — se re-armaba con pares frescos. Y la interpretación institucional es quizás la más elegante del bloque: es la firma estructural de la tabula rasa. En una legislatura con décadas de historia, las duplas de trabajo están atrincheradas — socios de comité de toda la vida, parejas de proyecto que se repiten congreso tras congreso — y un `gwdsp` positivo sería lo esperable. Aquí nadie se conocía con nadie: no existía el *stock* de sociedades heredadas, y el modelo lo detecta como ausencia de inercia relacional (aun cuando la familiaridad ya empezaba a operar en el margen, como muestra el RHEM ordenando la secuencia). ¿Contradice esto al RHEM (3.3), donde la familiaridad de pares es el mejor predictor? No — y el contraste es instructivo. El RHEM mira la *película*: compara cada coalición real contra coaliciones ficticias del mismo día, y encuentra que la familiaridad ordena la *secuencia* (quién arma el próximo equipo). El `gwdsp` mira la *foto final*: compara el stock total de repetición contra la referencia del propio modelo, y encuentra que ese stock no es excesivo. Ambas cosas son ciertas a la vez gracias a la regla 8--16: cada iniciativa obliga a coaliciones anchas, así que hasta las duplas más fieles llegan siempre acompañadas de pares nuevos — los mismos capitanes, con el equipo renovándose en cada partido.
