---
title: "Playground metodológico del proyecto"
subtitle: "RHEM, ERGM bipartito y diseños en exploración — introducciones pedagógicas y planes de implementación (antes: RHEM-intro)"
...
![Figura A. Indicaciones por fecha de informe y comisión: total fechado (claro) y eventos multi-firmantes (sólido).](../results/figures/indication_events_timeline.pdf){width=100%}
...
$$\Pr(informe_a = 1) = \Lambda\big(\alpha_{c(a)} + \beta^\top Z_a\big),$$

donde $a$ recorre las 947 iniciativas, $informe_a = 1$ si la iniciativa aparece en algún informe (el cruce plataforma$\to$TRACK vía `sources`/`icc_id`, con la cautela de formatos tipo "ICC N°411" ya catalogada), $\alpha_{c}$ son efectos fijos de comisión (las 120 sin comisión forman su propia categoría), y $Z_a$ es exactamente el bloque de covariables de coalición de M4, medido al momento de la firma: distancia del centro de la coalición al pívot, $sd(\theta_1)$ interna, tamaño, densidad interna (calculada con la red acumulada hasta la fecha de ingreso — la plataforma la fecha al 100%), proporción de abogados y de experimentados. Es un logit simple, barato, con EE por coalición. Dos productos: el efecto de selección en sí (¿qué coaliciones ni siquiera llegan?), y el insumo para re-estimar M4 con corrección de selección (Heckman-probit o, más simple y honesto, reportar M4 condicional e incondicional).
...
$$E_{a,t} = S_a \cup \bigcup_{s \le t} F_{a,s},$$

la unión de la coalición firmante génesis $S_a$ con los firmantes de todas las indicaciones que el artículo recibió hasta la onda $s \le t$. Las covariables de coalición de M4 se recalculan sobre $E_{a,t}$ en cada onda: $dist97_{a,t} = |\bar\theta_{1,E_{a,t}} - \theta_{1,(103)}|$, $sd(\theta_1)_{a,t}$, tamaño, densidad interna (con la red acumulada hasta $t$). El modelo es un *hazard* discreto — la versión dinámica natural del logit de 5.1:

$$\Pr\big(muere_{a,t} \,\big|\, vivo_{a,t-1}\big) = \Lambda\big(\alpha_{c(a)} + \delta_t + \gamma^\top X_{a,t-1}\big),$$

con $\delta_t$ efectos de onda (absorben el calendario: los plazos matan en masa) y $X_{a,t-1}$ el bloque de custodia rezagado una onda (lo que el equipo *era* antes del período en que el artículo sobrevive o muere). Como no observamos muerte onda a onda para todos los artículos (el desenlace fino es contra el borrador final), la variante mínima defendible es el modelo de *retención final con trayectoria*: $y_a$ (similitud con el borrador) sobre características de la trayectoria del equipo — cuánto creció, cuánto se ensanchó ideológicamente, si se acercó al pívot entre la génesis y la última onda.

## 12.4 Las tres preguntas nuevas que esto responde

1. ¿El ensanchamiento *sobreviene* o *preexiste*? La sección 5.1 muestra que las coaliciones anchas ganan; el panel distingue si los artículos que sobreviven *nacieron* anchos o *se ensancharon* por la vía de las indicaciones (el equipo de custodia estirándose hacia el pívot onda a onda).
2. ¿Quién rescata? Los enmendadores que se suman a un artículo ajeno son un acto de coalición observable y fechado — el análogo dinámico de la firma génesis, con su propia pregunta de formación (¿se suman los cercanos al pívot? ¿los familiares de la red?).
...
1. Script de crosswalk plataforma$\leftrightarrow$TRACK con auditoría de match (barato; desbloquea todo).
2. Etapa 0 (logit de llegada a informe; una tarde).
3. Panel artículo$\times$onda para C1/C3/C4/C6 con la variante de retención-con-trayectoria (el hazard discreto como extensión si el panel muestra señal).
...
# 13. El SDM y el veredicto sobre $\rho$: la discusión completa (2026-07-23)

Esta sección deja por escrito la discusión que tuvimos sobre el modelo espacial de Durbin del éxito individual (sección 5.2 del reporte) — qué nos dio, qué dice la literatura sobre cómo leerlo, y por qué decidimos **no venderlo como influencia** (y no llevarlo a la presentación).

## 13.1 Qué corrimos y qué salió

El modelo: $y = \rho W y + X\beta + WX\gamma + \varepsilon$, con $y$ = retención léxica media de los artículos co-firmados, $W$ = red de co-patrocinio normalizada por filas, ML. Resultados clave: $\rho = 0.891$ (robusto 0.63--0.95 entre variantes de $W$); los impactos exactos con $(I-\rho W)^{-1}$ denso dan directos chicos ($\approx$ OLS) y totales amplificados $\sim 9\times$ por el multiplicador $1/(1-\rho)$ — tomados literalmente, no creíbles (convertir a alguien en abogado "sumaría" $+1.86$ de retención a la sala cuando la media individual es $0.09$); la inferencia Monte Carlo de los totales explota (sorteos con $\rho \to 1$). Y la descomposición del "clima": $Wy$ se predice con los propios $X$ del ego con $R^2 = 0.81$ y correlaciona $-0.84$ con la distancia al pívot — el clima es mayormente *composición*; en los experimentos de gemelos, $\rho$ explica en promedio 38% de la varianza de las brechas pero solo 2--12% de las brechas grandes. Lo único que el paso OLS$\to$SDM aporta de sustancia: la distancia al pívot *migra* íntegra de la persona al vecindario (propia $p = 0.95$; del vecindario $-0.27^{*}$) — el éxito es del *lugar en el paisaje de coaliciones*, no del individuo.

## 13.2 El mapa de la literatura (verificado con los PDF, no de memoria)

- **Solo una tradición se ganó el derecho a leer $\rho$ como influencia**: Friedkin-Johnsen (1990; CUP 2011). Pagaron tres peajes que nosotros no podemos pagar: $W$ = comunicación *medida*, un modelo de proceso explícito (French-DeGroot), y validación experimental. Nuestra $W$ es co-firma — elegida por los actores, la misma conducta que modelamos como homofilia en RQ1.
- **La práctica moderna respetable ni reporta $\rho$ como titular**: Franzese-Hays (2007), Ward-Gleditsch (2008), LeSage-Pace (2014, "The Biggest Myth in Spatial Econometrics": los coeficientes no son efectos; repórtense impactos), Whitten-Williams-Wimpy (2021: solo 42% de los papers con SAR va más allá del "$\rho$ grande y significativo"). Nosotros ya calculamos impactos exactos — estamos del lado correcto de esa crítica.
- **Las críticas duras aplican de lleno**: Gibbons-Overman (2012, "Mostly Pointless Spatial Econometrics?"): sin $W$ conocida y exógena la identificación es "at best very weak... unless the main aim is description". Shalizi-Thomas (2011): con red elegida por los actores, influencia y homofilia (incluida la latente) son *genéricamente* inseparables en corte transversal. Betz-Cook-Hollenbach (2018): los instrumentos espaciales ($WX$ como IV de $Wy$) son inválidos por construcción — no hay arreglo por 2SLS espacial.
- **Dos escudos a favor**: Mizruchi-Neuman (2008/2010): el sesgo de muestra finita del ML de $\rho$ es *negativo* y crece con la densidad — un $\rho$ alto no es artefacto del estimador. Y la banda 0.63--0.95 entre variantes de $W$ es exactamente el ejercicio de sensibilidad que Leenders (2002) y Juhl (2020) exigen.
- **El dato duro**: no encontramos ningún trabajo publicado que venda un $\rho \approx 0.9$ sobre co-firmas como influencia. Un lector metodológico ve multiplicador $\approx 9$ (borde de la no-estacionariedad) y piensa "shocks comunes o acoplamiento mecánico".

## 13.3 El flag propio: parte de $\rho$ es aritmética

Nuestro $y$ es la retención media de los artículos co-firmados: dos co-firmantes comparten literalmente artículos — es decir, *componentes del outcome*. Parte del acoplamiento es identidad contable, no proceso social. Es cuantificable ("$\rho$ des-mecanizado"): recalcular el éxito de los vecinos excluyendo los artículos compartidos con el ego y ver cuánto $\rho$ sobrevive. Script corto, pendiente post-charla; la cifra vale más que un párrafo de cautelas.

## 13.4 La ruta Battaglini (AJPS 2020), leída completa: el upgrade posible

Battaglini, Leone Sciabolazza y Patacchini estudian *exactamente* nuestro problema (co-firmas del Congreso $\to$ efectividad legislativa) y su ecuación estimante *es* un SAR — pero con dos cosas montadas alrededor: (i) **microfundación**: producción cooperativa con complementariedad de esfuerzo, equilibrio Nash único donde la efectividad de cada uno es su centralidad de Katz-Bonacich; (ii) **estrategia de endogeneidad**: primera etapa diádica de formación de la red ($g_{ij}$ = % de proyectos de $i$ co-firmados por $j$, sobre |diferencias| de atributos **más la red alumni** — misma institución $\pm 4$ años, coef $+4.09^{***}$, F $= 5.138$), y los residuos de esa etapa sumados por persona entran como función de control en la ecuación de efectividad (NLLS; bootstrap residual espacial 500 reps). $\phi$: 0.44 sin corrección $\to$ 0.58--0.83 con ella. Hasta hacen la variante con **ERGM como primera etapa** (GWESP + GWIDegree, distribución simulada de $\phi$ toda positiva) — la etapa que nosotros ya tenemos construida.

La receta adaptada a la Convención (agenda de paper, no de charla): (1) outcome *atribuible*: retención de las iniciativas como **primer firmante** ($\sim 6$ por persona — mata el acoplamiento mecánico de 13.3); (2) $W$ dirigida $g_{ij}$ = % de iniciativas de $i$ firmadas por $j$; (3) primera etapa = nuestros modelos de formación (clogit / ERGM bipartito) más la **red alumni chilena** (universidad $\times$ cohorte desde las biografías BCN — un día de colección para 154 personas); (4) función de control + SAR por NLLS con bootstrap de iniciativas; (5) el framing Katz-Bonacich. Límites honestos: ellos tienen el LES (métrica externa validada), 5 congresos y $n = 2.176$ — nosotros un corte de 154 (poder modesto, todo por bootstrap); en Chile la universidad es más señal ideológica que en EE.UU., así que la exclusión del alumni solo es defendible condicional en $\theta_1/\theta_2$; y su teoría de favores vive del juego repetido — aunque eso es también nuestro ángulo propio: probar la complementariedad de esfuerzo *sin sombra del futuro*.

## 13.5 La decisión

$\rho$ se lee y se escribe como **interdependencia/aglomeración del éxito estructurada por la red de co-firma** (consistente con "el éxito es un bien de coalición") — nunca "influencia", nunca "contagio"; el peso sustantivo va en los impactos y en la migración OLS$\to$SDM de la distancia al pívot. En la presentación del 23-jul: **nada de SDM** (se retiró también del backup). En el reporte queda la sección 5.2 con su lectura descriptiva; pendientes de decisión: el párrafo de límites con las citas de 13.2, el $\rho$ des-mecanizado (13.3) y la ruta Battaglini (13.4) como diseño en la revisión crítica.
