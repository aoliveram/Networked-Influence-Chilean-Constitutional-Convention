...
\begin{frame}{Índice}
\begin{itemize}
  \item[{\color{ndred}\textbf{1.}}] La Convención
...
\begin{itemize}
  \item En una legislatura ordinaria, las redes de colaboración son el producto de décadas: partidos, comités, favores, etc.
  \vspace{0.35cm}
  \item La Convención Constitucional chilena (2021--2022) ofrece lo que casi nunca se observa: \emphr{colaboración política desde una tabula rasa}.
  \vspace{0.35cm}
  \item Mayoría de novatos, órgano que se disuelve al terminar, reglas escritas por sus propios miembros.
  \vspace{0.35cm}
  \item Tenemos \emphr{registro documental (casi) completo} de cada firma de iniciativa, indicación y voto en pleno.
\end{itemize}
\end{frame}
...
\begin{itemize}
  ...
  \vspace{0.25cm}
  \item El órgano \emphr{se disolvía al entregar el texto}.
  \vspace{0.25cm}
...
\begin{itemize}
  ...
  \item \emphr{Iniciativas}: 995 ingresadas a la plataforma oficial (nov-21 -- feb-22; \emphr{947} con 2--16 firmantes); 100\% fechadas. \\
  1.565 artículos génesis (supervivencia 20\%), con historial fechado de indicaciones.
...
\vspace{0.2cm}
\begin{itemize}
  \item \nota{Iniciativas = con 2--16 firmantes asignadas a la comisión (otras 120 sin comisión asignada).} 
  \item \nota{Multifirm. = indicaciones con $\geq 2$ firmantes, las únicas que agregan lazos.}
  \item \nota{El cero de C5 es limitación de registro.}
\end{itemize}
\end{frame}

\begin{frame}{Las preguntas de investigación}
\begin{itemize}
  \item El registro documental completo permite muchas preguntas. Hoy abordaremos solo algunas.
  \vspace{0.3cm}
...
\vspace{0.05cm}
\nota{\emphr{Esta es la unidad de análisis real}: convencional $\times$ documento.} \\
\nota{La proyección persona-persona de una iniciativa de 16 firmantes fabricaría 120 pares de una vez.}
\end{frame}

\begin{frame}{Red de colaboración: 2}
\fig{0.95}{indication_events_timeline.pdf}
\vspace{0.05cm}
\nota{Cada barra es un día de informe de comisión.} \\
\nota{Sólido = eventos multi-firmantes; Claro = total fechado.}
\end{frame}
...
\begin{itemize}
  ...
  \begin{itemize}
    \item Esa ventana es anterior a las comisiones (oct), a las iniciativas (nov) y a la regla de 2/3 operativa (feb).
  \end{itemize}
  \vspace{0.3cm}
  \item \emphr{Voto revelado dinámico}: dynIRT sobre las 4.707 votaciones $\to$ trayectoria $\theta_{i,t}$ en 91 períodos.
\end{itemize}
\end{frame}
...
\begin{itemize}
  ...
  \vspace{0.25cm}
  \item Las dos lentes \emphr{convergen en el qué} — y la herramienta correcta \emphr{cambia el cuánto y revela lo que solo la estructura ve}:
  \begin{itemize}
...
\begin{itemize}
  \item Utilidad latente de firmar; lo que se estima es la \emphr{probabilidad condicional} de la coalición observada, dado su tamaño:
\end{itemize}
...
\begin{itemize}
  \item 947 menús $\times$ 154 candidatos = 145.838 decisiones.
  \item Costo: no modela la estructura; solo ve la elección \emph{dado} que el evento existe.
  \item Covariables: ideología (2D), lista electoral, distrito/pueblo, perfil pre-Convención (abogado, experiencia, edad, educación, género).
\end{itemize}
\end{frame}
...
\vspace{0.25cm}
\begin{itemize}
  \item La izquierda organizada coordina por lista e ideología, no por geografía; el género vive en la centro-izquierda.
  \item La derecha casi no cruza distancias.
\end{itemize}
\end{frame}
...
{\footnotesize
$$s(y) = \Big(\underbrace{\sum_{i,a} y_{ia}\,\mathbf{1}[i \in M_a]}_{\text{firmas de miembros}},\;\;
\underbrace{\sum_{a}\sum_{i<j} y_{ia}\,y_{ja}\,\mathbf{1}[\ell_i = \ell_j]}_{\text{pares de misma lista}},\;\;
\underbrace{\sum_{a}\big(\max_{i:\,y_{ia}=1}\theta_{1i} - \min_{i:\,y_{ia}=1}\theta_{1i}\big)}_{\text{rango ideológico por documento}},\;\dots\Big)$$}
\begin{itemize}
  \item Cada término pregunta: ¿este patrón aparece \emphr{más que en redes aleatorias comparables}?
  \item El denominador $\kappa(\theta)$ suma sobre \emph{todas} las redes posibles ($2^{154 \times n}$) — incalculable; simularlo (MCMC) resultó infactible.
  \item MPLE lo \emphr{esquiva} (cada lazo, condicional al resto); EE por \emphr{bootstrap de iniciativas} (B = 500), no inflados.
\end{itemize}
\end{frame}

\begin{frame}{La misma pregunta con la estructura completa}
\begin{center}\tiny\setlength{\tabcolsep}{3pt}\renewcommand{\arraystretch}{1.25}
...
\end{center}
\vspace{0.05cm}
\nota{La ideología opera en ambas dimensiones; la lista se achica al condicionar en estructura.} \\
\nota{Las \emph{parejas no se sobre-usan} (gwdsp $<$ 0 en las 7).}
\end{frame}

\begin{frame}{Qué aprendimos con la unidad real}
\begin{itemize}
  \item La unidad de análisis real es \emphr{persona $\times$ documento}: cada firma es una decisión sobre un texto concreto, no un lazo proyectado entre personas.
  \vspace{0.35cm}
  \item Diferencias con el CLogit: la \emphr{lista se achica} (parte era estructura de actividad), el \emphr{distrito se apaga en C4}, y el género pierde su marginal.
  \vspace{0.35cm}
  \item Un hecho que el CLogit no puede ver: las \emphr{parejas no se sobre-usan} (gwdsp $<$ 0 en las 7) — la firma estructural de la \emphr{tabula rasa}: sin inercia institucional no hay duplas atrincheradas; las coaliciones se re-arman con pares frescos.
\end{itemize}
\end{frame}
...
\begin{itemize}
  \item Efectos fijos individuales (solo variación \emph{dentro} de cada persona); 4.355 obs.
...
\begin{frame}{La exposición con decaimiento temporal (ventana completa)}
\begin{center}\small
\begin{tabular}{lcccc}
\toprule
Definición de exposición & $\hat\lambda$ & EE & $p$ & N \\
\midrule
Decaimiento $\lambda_w = 0.25$ & $+0.008$ & 0.004 & $0.051$ & 4.355 \\
Decaimiento $\lambda_w = 0.50$ & $+0.008$ & 0.004 & $0.042$ & 4.355 \\
Decaimiento $\lambda_w = 0.75$ & $+0.007$ & 0.004 & $0.078$ & 4.355 \\
\bottomrule
\end{tabular}
\end{center}
\vspace{0.25cm}
\begin{itemize}
  \item Con memoria que descuenta el pasado, el lag roza la significancia ($\lambda_w = 0.50$: $p = .042$).
  \item Pero la falsificación es igual de "significativa" ($+0.010$, $p = .030$): la firma de la selección.
  \item La tabla completa (3 ventanas $\times$ 7 dosis) queda en backup.
\end{itemize}
\end{frame}

\begin{frame}{A considerar antes de declarar nulo el efecto de influencia}
\begin{enumerate}
  \item La distancia promedio entre cada convencional y su vecindario es $0.59$ (contra $2.25$ si los vecindarios fueran al azar).
  \vspace{0.25cm}
