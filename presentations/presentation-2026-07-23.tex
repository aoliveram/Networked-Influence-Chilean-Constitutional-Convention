...
\begin{itemize}
  ...
  \vspace{0.25cm}
  \item Las dos lentes están de acuerdo, pero la herramienta correcta \emphr{cambia la magnitud y revela} el factor de estructura:
  \begin{itemize}
...
\begin{itemize}
  ...
  \vspace{0.35cm}
  \item Diferencias con el CLogit: 
  \begin{itemize}
    \item La \emphr{lista se achica} (parte era estructura de actividad).
    \item El \emphr{distrito se apaga en C4}, y el género pierde su marginal.
  \end{itemize}
  \vspace{0.35cm}
  \item Un hecho que el CLogit no puede ver: las \emphr{parejas no se sobre-usan} (gwdsp $<$ 0 en las 7).
  \item La firma estructural de la \emphr{tabula rasa}: sin inercia institucional no hay duplas estables que sobrevivan a las covariables.
\end{itemize}
\end{frame}
...
\vspace{0.05cm}
\nota{(a) Entre personas: posición y vecindario van pegados ($r=0.95$) — elegimos vecindarios que se nos parecen. (b) Dentro de cada persona, los cambios no se siguen.} \\
\nota{Ámbar: el 5\% más desalineado cae en (b) sin pendiente ($r=-0.04$).}
\end{frame}
...
\begin{enumerate}
  ...
  \vspace{0.25cm}
  \item $\theta$ se estima desde los votos, con error (grande: 3.6$\times$ el movimiento típico entre períodos). Lo propagamos: re-simulamos las votaciones 50 veces, re-estimamos $\theta$ y el modelo completo en cada réplica. El error estándar de $\lambda$ es similar ($0.0036 \to 0.0039$): el nulo no es un artefacto de medición.
\end{enumerate}
\end{frame}
...
\begin{frame}{RQ2b: el resultado central}
\begin{center}\small
\begin{tabular}{lcc}
\toprule
 & $\phi$ & $p$ \\
\midrule
Exposición a defectores (real) & $+11.2$ & $<10^{-100}$ \\
Esperado por pura mecánica (permutación, 200 réplicas) & $+6.0$ [p95: $6.1$] & $<0.005$ \\
\bottomrule
\end{tabular}
\end{center}
\vspace{0.25cm}
\begin{itemize}
  \item La defección viaja más fuerte \emph{entre} comisiones.
\end{itemize}
\end{frame}
...
\begin{itemize}
  ...
  \item La co-defección viaja por los \emphr{pares novatos}.
\end{itemize}
\end{frame}
...
\begin{itemize}
  \item La unidad es el \emph{artículo} (1.565, con coalición firmante $S_a$ de 2--16). 
  \item Tasa de supervivencia: 20.2\% logra llegar al borrador.
\end{itemize}
\vspace{0.5cm}
...
\begin{itemize}
  \item $C_{S_a}$ = bloque de red de la coalición (betweenness media, constraint media, densidad interna).
  \item $H_{S_a}$ = capital humano (prop.\ abogados, experiencia, grado medio).
\end{itemize}
\end{frame}
...
\begin{frame}{Los artículos separados por quintiles de posición media de la coalición}
\begin{center}\small
\begin{tabular}{lccc}
\toprule
...
\vspace{0.25cm}
\begin{itemize}
  \item El decaimiento monótono es una estampa de la lógica de los 2/3 votos.
\end{itemize}
\end{frame}
...
\begin{frame}{Resumen}
\begin{itemize}
  \item La red de colaboración \emphr{se podía predecir} con lo pre-existente: lista, ideología y territorio. 
  \vspace{0.25cm}
  \item Las credenciales no la tejieron: los abogados no muestran homofilia generalizada.
  \vspace{0.25cm}
  \item Las \emphr{posiciones}: con exposición con decaimiento hay \emphr{indicios de influencia} ($p \approx .04$) — pero la falsificación también dispara, y separarlos exige análisis \emphr{aún en curso}: horse race contra la \emph{innovación} del futuro, $\theta$ re-estimado solo con la era de normas, sub-ventanas.
  \vspace{0.25cm}
  \item La \emphr{conducta sí viaja}: la defección se agrupa por los lazos de co-firma.
  \vspace{0.25cm}
  \item Los \emphr{textos ganan por "geometría"} (el premio a ensancharse decae hacia el pívot de 2/3) y por equipos con historia interna.
\end{itemize}
\end{frame}
